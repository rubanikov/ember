...
def create_vectorized_features(data_dir, feature_version=2):
    ...
    raw_feature_paths = [os.path.join(data_dir, "train_{}.jsonl".format(i)) for i in range(3)]
    ...
    raw_feature_paths = [os.path.join(data_dir, "test_{}.jsonl".format(i)) for i in range(4)]
    nrows = sum([1 for fp in raw_feature_paths for line in open(fp)])
    vectorize_subset(X_path, y_path, raw_feature_paths, extractor, nrows)
...
def create_metadata(data_dir):
    ...
    train_feature_paths = [os.path.join(data_dir, "train_{}.jsonl".format(i)) for i in range(3)]
    ...
    test_feature_paths = [os.path.join(data_dir, "test_{}.jsonl".format(i)) for i in range(4)]
